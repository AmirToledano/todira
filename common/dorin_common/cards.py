...
# A listing with zero real photos gets a staged photo of Todi — the user's own dachshund — instead
# (2026-09-02 request; see PROJECT_STATE.md for the history of earlier approaches this replaced:
# a CC0 cartoon illustration, real photos with background/people removed, real photos with an
# emoji over any visible face — all rejected in favor of this). These 18 photos were generated by
# the user directly (Gemini "nano banana" image editing) and are used as-is; picked FULLY AT
# RANDOM per send (2026-09-02 request: "שיהיה אקראי לחלוטין") rather than deterministically per
# listing. Same treatment on the website — see website/templates/_listing_card.html.
_DACHSHUND_DIR = Path(__file__).resolve().parent / "assets" / "dachshunds"
_TODI_PHOTO_COUNT = 18
_NO_PHOTOS_SUFFIX_HE = "\n\n🐶 <i>דירה זו עלתה ללא תמונות, אבל הנה טודי בשבילכם</i>"


def _dachshund_photo_path() -> Path:
    n = random.randint(1, _TODI_PHOTO_COUNT)
